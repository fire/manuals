...
Transferred from the archived `weftspun/interactor-seethrough-ggml` repository, originally `docs/decisions/0008-open-work.md`. The repository is archived and read-only; this copy is the one that stays maintained. Content is verbatim apart from the front matter, which replaces the original heading and status line.
:::
...
Every escaped defect so far lived in the pipeline DAG's _configuration
space_, not in any single op: backend (Vulkan/CPU) × knob (flash attention,
